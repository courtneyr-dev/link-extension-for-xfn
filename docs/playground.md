...
- **`blueprint.json`** (repo root) — an older variant that installs the plugin and lands on a new draft post instead of the demo page. Kept for reference; not deployed. Reconciling the two is flagged in the [documentation plan](documentation-plan.md).
