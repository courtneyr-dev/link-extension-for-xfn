...
8. **Nonce without a verifier:** the editor script is localized with an `xfn_link_extension` nonce that no server endpoint checks. Confirm no endpoint is missing.
...
12. **Demo page loses its `rel` attributes.** Running the demo steps from `assets/blueprints/blueprint.json` on a local WordPress 7.0.1 Playground publishes `/xfn-demo/`, but the rendered page's links carry no `rel` attributes — so tooltips can't appear there even on WordPress 7.0+, and the demo doesn't demonstrate the plugin's output. Verify whether the blueprint content or a save-time filter strips them.
