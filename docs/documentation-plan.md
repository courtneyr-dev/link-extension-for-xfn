...
5. **Playground blueprints — partly resolved 2026-08-20.** `assets/blueprints/blueprint.json` is authoritative: it is what `deploy-to-wordpress-org.sh` and `deploy-blueprint-only.sh` publish, and what the listing's Live Preview loads. `blueprint-improved.json` was byte-identical to it and has been deleted. The root `blueprint.json` is **not** a copy — it lands on `/wp-admin/post-new.php` instead of the `/xfn-demo/` page and has its own login step — and nothing in the repo references it, though it does ship in `trunk/`. It also still writes `enable_floating_toolbar`, a setting removed in 1.0.4, so anything loading it sets a dead option. Decide whether it has a consumer and delete it if not; left in place for now rather than dropping a shipped file mid-release.
