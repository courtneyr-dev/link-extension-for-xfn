# Maintaining the Link Extension for XFN documentation

The user docs site at <https://courtneyr-dev.github.io/link-extension-for-xfn/> builds from this directory with [Astro Starlight](https://starlight.astro.build/). This guide covers day-to-day updates. The full cross-plugin standard (page templates, writing style, accessibility and SEO rules) lives in the maintainer's docs workspace: `plugin-docs/wiki/standards/plugin-docs-standard.md`.
...
- Everything else at `docs/` root (developer notes, QA guides, historical reports, this file) is **not** published.
...
2. Internal links are root-relative and include the base path: `/link-extension-for-xfn/settings/`.
...
cd docs
npm ci                 # first time
npm run dev            # live preview at localhost:4321/link-extension-for-xfn/
